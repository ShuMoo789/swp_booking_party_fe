...
function PackageItem(props) {
  ...
  const fetchSchedule = async () => {
    const response = await api.get(`/schedule-by-host-id/${props.hostId}`);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* </Link> */}
        <Modal open={showBooking} onOk={() => form.submit()}>
          <Form
            form={form}
            onFinish={async () => {
              const response = await api.get(`/order123?orderId=${v4()}`);
...
function PackageList() {
  ...
  const fetchPackage = async () => {
    const response = await api.get(`/packageUpload-by-host-id/${params.id}`);
